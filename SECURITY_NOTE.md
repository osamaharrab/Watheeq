...
The implemented health and readiness endpoints accept no business payload and report service or dependency status. Seed ingestion and ownership projection are local Django management-command paths, not public HTTP endpoints. The projection clears and rebuilds the derived ownership graph from PostgreSQL and applies uniqueness constraints to the two projected identifiers. Reconciliation and projection drift detection remain unimplemented. Weaviate built-in vectorization and modules are disabled with `DEFAULT_VECTORIZER_MODULE: none` and `ENABLE_MODULES: ""`.
